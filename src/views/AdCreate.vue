...
export default {
  ...
  methods: {
    async loadData2() {
      await this.$axios.get(`/api/ad/${this.id}`).then((res) => {
    ...
    async addSubmit(e) {
      ...
      if (this.id) {
        this.$refs.createFormRef.validate(async (valid) => {
          ...
          await this.$axios.put(`/api/ad/${this.id}`, body);
        });
      } else {
        this.$refs.createFormRef.validate(async (valid) => {
          // console.log(this.createForm);
          ...
          await this.$axios.post("/api/ad", body);
        });
      }
      // if (!data) {
    ...
    async handleImageAdded(file, Editor, cursorLocation, resetUploader) {
      ...
      const res = await this.$axios.post("/api/ad/upload", formData);
