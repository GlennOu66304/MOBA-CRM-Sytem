...
export default {
  ...
  methods: {
    async loadData2() {
      await this.$axios.get(`/api/hero/${this.id}`).then((res) => {
        // undefined fix
    ...
    async loadItems() {
      await this.$axios.get("/api/item").then((res) => {
        // console.log(res);
        this.items = res.data;
      });
    },
    async addSubmit(e) {
      ...
      if (this.id) {
        this.$refs.createFormRef.validate(async (valid) => {
          ...
          await this.$axios.put(`/api/hero/${this.id}`, body);
        });
      } else {
        this.$refs.createFormRef.validate(async (valid) => {
          console.log(this.createForm);
          ...
          await this.$axios.post("/api/hero", body);
        });
      }
      // if (!data) {
