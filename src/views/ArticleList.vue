  ...
  <div class="ArticleList">
    ...
    <el-table :data="tableData" border width="100%">
      ...
      <el-table-column prop="title" label="标题" width="120" align="center">
       ...
       <el-table-column prop="categories.length" label="上级" width="120" align="center">
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-button
            size="mini"
            type="primary"
            @click="$router.push(`/article/create/${scope.row._id}`)"
...
export default {
  ...
  methods: {
    async loadData() {
      await this.$axios.get(`${process.env.VUE_APP_API}/api/article`).then((res) => {
    ...
    async handleDelete(id) {
      ...
      await this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(async () => {
          await this.$axios.delete(`${process.env.VUE_APP_API}/api/article/${id}`).then((res) => {
