<template>
  <div class="HeroList">
    ...
    <el-table :data="tableData" border width="100%">
      ...
      <el-table-column prop="title" label="称号" width="120" align="center">
      </el-table-column>

      <el-table-column prop="avatar" label="头像" width="120" align="center">
        <template slot-scope="scope">
          <img :src="scope.row.avatar" style="height: 30px" alt="" />
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-button
            size="mini"
            type="primary"
            @click="$router.push(`/hero/create/${scope.row._id}`)"
...
export default {
  ...
  methods: {
    async loadData() {
      await this.$axios.get(`${process.env.VUE_APP_API}/api/hero`).then((res) => {
    ...
    async handleDelete(id) {
      ...
      await this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(async () => {
          await this.$axios.delete(`${process.env.VUE_APP_API}/api/hero/${id}`).then((res) => {
